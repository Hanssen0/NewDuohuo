<template>
  <div id="viewport-cutter">
    <div id="background" :style="{transform: 'translateX(' + position + 'px)',
                                  transition: 'transform ' + transition_time +
                                              's linear'}"
      ref="content" @transitionend="InvertBackgroundDirection">
...
<!-- Repeat once for infinite scrolling -->
    </div>
  </div>
</template>
<style scoped>
...
<script>
const kTransitionTime = 20;
export default {
  data: function() {
    return {
      projects: new Array(),
      position: 0,
      transition_time: kTransitionTime
    };
  }, methods: {
...
console.log("F");
      ...
      this.$nextTick(() => {
        this.position = 0;
        this.$nextTick(() => {
          this.transition_time = kTransitionTime;
          this.position = -this.$refs["content"].offsetWidth/2;
        });
      });
    }
  }, mounted: function() {
    ...
    this.$nextTick(() => { this.position = -this.$refs["content"].offsetWidth/2; });
  }
}
</script>
